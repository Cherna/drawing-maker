...
export class Effects {
    ...
    /**
     * Cleanup pass: Removes all paths shorter than the specified length.
     * Useful for removing noise and tiny dots generated by hatching.
     */
    static cleanup(model: MakerJs.IModel, minLength: number) {
        const filterSmallPaths = (m: MakerJs.IModel) => {
            if (m.paths) {
                // Sort keys for determinism
                const keys = Object.keys(m.paths).sort();
                for (const id of keys) {
                    ...
                    try {
                        ...
                        // Sometimes measurement fails on degenerate curves
                        delete m.paths[id];
                    }
                }
            }
            if (m.models) {
                const keys = Object.keys(m.models).sort();
                for (const key of keys) {
                    filterSmallPaths(m.models[key]);
                }
            }
        };

        filterSmallPaths(model);
        return model;
    }
